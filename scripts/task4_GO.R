...
write.csv(human_tbl,
          ...
          row.names = FALSE)
write.csv(conserved_hm_tbl,
          file.path(outdir, "conserved_human_in_mouse_rGREAT_GO_BP.csv"),
          row.names = FALSE)
write.csv(conserved_mh_tbl,
          file.path(outdir, "conserved_mouse_in_human_rGREAT_GO_BP.csv"),
          row.names = FALSE)
...
conserved_mh_tbl2 <- standardize_tbl(conserved_mh_tbl, "conserved_mouse_in_human")

# -----------------------------
# 9. Keep significant terms
# -----------------------------
mouse_sig        <- mouse_tbl2        %>% filter(!is.na(padj_final), padj_final < 0.05) %>% arrange(padj_final)
human_sig        <- human_tbl2        %>% filter(!is.na(padj_final), padj_final < 0.05) %>% arrange(padj_final)
conserved_hm_sig <- conserved_hm_tbl2 %>% filter(!is.na(padj_final), padj_final < 0.05) %>% arrange(padj_final)
conserved_mh_sig <- conserved_mh_tbl2 %>% filter(!is.na(padj_final), padj_final < 0.05) %>% arrange(padj_final)

write.csv(mouse_sig,        file.path(outdir, "mouse_specific_rGREAT_GO_BP_sig.csv"),        row.names = FALSE)
write.csv(human_sig,        file.path(outdir, "human_specific_rGREAT_GO_BP_sig.csv"),        row.names = FALSE)
write.csv(conserved_hm_sig, file.path(outdir, "conserved_human_in_mouse_rGREAT_GO_BP_sig.csv"), row.names = FALSE)
write.csv(conserved_mh_sig, file.path(outdir, "conserved_mouse_in_human_rGREAT_GO_BP_sig.csv"), row.names = FALSE)

# -----------------------------
# 10. Plot helper function
# -----------------------------

make_dotplot <- function(sig_list, dataset_levels, filename, ncol = 2, width = 13, height = 7) {
  combined <- bind_rows(sig_list)
  ...
  tb2$dataset <- factor(tb2$dataset, levels = dataset_levels)
  ...
  p <- ggplot(tb2, aes(x = neglog10_padj, y = term_plot)) +
    ...
    scale_fill_gradient(
      low = "#DCE6F2", high = "#4C78A8",
      name = expression(-log[10]("adj. p"))
    ) +
    scale_size_continuous(range = c(2.5, 7), name = "Fold enrichment") +
...
make_dotplot(
  ...
  filename       = "dotplot_conserved.png",
  ncol = 2, width = 13, height = 7
)
